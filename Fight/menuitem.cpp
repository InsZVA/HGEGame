...
hgeGUIMenuItem::hgeGUIMenuItem(int _id, HEFFECT _snd, float _x, float _y, float _delay, char *_title)
{
	float w;
	
	id=_id;
	fnt=new GfxFont("¿¬Ìå",32);
	snd=_snd;
	delay=_delay;
	title=_title;
	...
	w=32*strlen(title);
	rect.Set(_x-w/2, _y, _x+w/2, _y+32);
}
...
void hgeGUIMenuItem::Render()
{
	...
	fnt->Print(rect.x1+offset+3, rect.y1+3, title);
	fnt->SetColor(color.GetHWColor());
	fnt->Print(rect.x1-offset, rect.y1-offset, title);
}
